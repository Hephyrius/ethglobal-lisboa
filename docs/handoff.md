...
- **Archetype cards on `/create`** — one click, no form. An *archetype* is a set of **bounds**
  (`packages/schema/archetypes/`); the model writes a fresh mandate inside them per click, so two
  clicks give two different vaults. Distinct from the **presets** below it, which load one fixed
  mandate. The word meant "preset" until this wave — the heading was renamed so it does not mean two
  things on one page. Every bound line is generated by `describeEnvelope()`, so no number on a card
  is typed by this lane.
- **Paused banner** — states the halt and the exit with equal weight. "Paused" reads as *your money
  is stuck* and here it is the opposite by construction, so the withdrawal side is spelled out rather
  than left to be inferred from the deposit panel still working.
- **Withdraw is capped on what the vault can pay** (Lane A's #76, actioned). `maxWithdraw()` reports
  the *claim*, not what is payable. Live on the demo vault: 2,500.63 total against 985.20 in cash, so
  a full position was **39.4% redeemable** and the rest reverted from the ERC-20 — reading as a
  broken vault rather than a deployed one. `redeemInKind` is offered only while paused, simulated
  before signing.
- **My vaults** — vaults you *deployed*, separate from vaults you *hold shares in*. Never merged:
  an archetype vault has no deposit, so it appears only in the first until funded.
- **Every `SourceNote` renders.** That field had been displayed nowhere since Lane C shipped
  diagnostic notes in Wave 2 — a populated array nobody read, with no error and no empty state.

⚠️ **Two live gotchas for whoever runs the demo.**

1. **A dev server bakes in `NEXT_PUBLIC_*` at start.** One launched with a throwaway
   `NEXT_PUBLIC_API_URL` keeps serving that dead port after `.env.local` is fixed — answering 200 on
   every route and falling back to golden fixtures on all of them (#99). If the badge says
   `FIXTURES` while the API is up, restart the server before debugging the code.
2. **Do not run two `next dev` on `web/`** — they contend for `.next` and the second never finishes
   compiling. The spare-port trick that works for a uvicorn does not work here.
